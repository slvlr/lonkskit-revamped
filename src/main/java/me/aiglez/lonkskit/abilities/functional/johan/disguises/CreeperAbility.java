    ...
    public void registerListeners() {
        Events.subscribe(EntityDamageByEntityEvent.class)
                ...
                .handler(e ->{
                    if (times2 == 0) {
                        e.setCancelled(true);
                        Bukkit.getScheduler().runTaskLater(KitPlugin.getSingleton(), () -> e.getEntity().setVelocity(e.getEntity().getLocation().getDirection().multiply(getConfiguration().getNode("creeper-launch").getDouble(1.5D)).setY(3D)),1L);
                    }
                    if (times2 >= 10) {
                        times2 = 0;
                    }else
